...
**Practical reading.** TCC 2 was defended on 2026-07-03 and the final version was submitted (see `docs/ESTADO_TCC2_2026-07-10.md`). Convalidation is a mechanism for *replacing* the TCC deliverable, so once TCC 2 is approved there is nothing left to convalidate. Do not plan the paper around Art. 33–34 unless the component is still open for some reason — `TODO(verify)` the current academic status with DERAC/PRATCC before relying on either reading. What Art. 34 §3º still tells you is how the institution ranks venues, which is a reasonable proxy for where the effort is best spent: A-stratum venues are worth materially more than B ones.
