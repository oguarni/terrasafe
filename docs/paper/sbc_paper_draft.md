...
Analisadores híbridos que combinam regras determinísticas com aprendizado de máquina são comuns em Infraestrutura como Código, mas a contribuição do componente aprendido raramente é isolada e mais raramente ainda é relatada quando é desfavorável. Avaliamos o TerraVault, analisador estático open source para Terraform que combina 11 regras (60% do score de risco) com um Isolation Forest sobre um feature vector estrutural de 8 dimensões (40%). Em um corpus controlado de 22 módulos, ele atinge 100/100/100 de precisão/recall/F1 sob uma taxonomia compartilhada de 11 categorias, contra 100/95,7/97,8 (Checkov), 100/87,0/93,0 (tfsec) e 100/47,8/64,7 (Terrascan). Em 57 *fixtures* de terceiros do KICS — que não está entre as ferramentas comparadas, de modo que nenhuma joga em casa — o TerraVault cai para 70,4/59,4/64,4, e o resultado é a decomposição: 83% de recall (19/23) dentro do escopo de recursos que declara cobrir e 0/9 fora dele, o que localiza a lacuna em amplitude de cobertura e não em qualidade de detecção. Uma ablação mostra então que o componente aprendido *comprime* a separação das regras no corpus controlado, de 33,3 para 21,4 pontos (score de anomalia isolado: 3,2). Isso não é revertido pela medição final, mas delimitado por ela: entre 18.041 configurações reais, nas 437 em que as regras nada reportam, a taxa de sinalização cresce monotonicamente ao longo de um eixo independente de atipicidade de Mahalanobis (2,0% a 100%, *lift* de 50,25x, AUC 0,9151), e o modelo dispara em 37,5% das configurações *rule-clean* contra 8,9% das *rule-flagged*. A metade aprendida não afia as regras onde estas já falam; acrescenta um sinal de revisão ortogonal onde elas se calam — sujeito ao limite de que atipicidade estrutural é forma de grafo, não evidência de vulnerabilidade.

**Palavras-chave:** Infraestrutura como Código, Terraform, SAST, detecção de anomalias, avaliação empírica, DevSecOps.
